import { Address, PScriptContext, ScriptType, Credential, Script, compile, pfn, unit, plet, pmatch, perror, PMaybe, data, pBool, passert, pstruct, PPubKeyHash, int, punsafeConvertType, bs, padd, PTxInfo, PTxOut, pfilter, Term, list, pInt, pfoldl, PAddress, PTxInInfo, pfind, PTxOutRef, pStr, pBs } from "@harmoniclabs/plu-ts";
...
const get_ada_from_outputs = pfn(
  [ list( PTxOut.type ) ], int
)(( outputs ) => {
  // iterate through outputs and sum up values
  return pfoldl(PTxOut.type, int).$(
    (acc, out) => {
...
const get_ada_from_inputs = pfn(
  [ list( PTxInInfo.type ) ], int
)(( inputs ) => {
  // iterate through inputs and sum up values
  return pfoldl(PTxInInfo.type, int).$(
    (acc, out) => {
...
const get_outputs_by_key = pfn(
  [ PTxInfo.type, PPubKeyHash.type ], list( PTxOut.type ))
(( tx, key ) => {
  // filter outputs whose address.credential.pkh equals the provided key
  return pfilter( PTxOut.type ).$(( output: Term<typeof PTxOut> ) => {
...
const get_inputs_by_addr = pfn(
  [ PTxInfo.type, PAddress.type ], list( PTxInInfo.type ))
(( tx, addr ) => {
  // filter inputs whose address equals the provided address
  return pfilter( PTxInInfo.type ).$(( input: Term<typeof PTxInInfo> ) => {
...
(({ redeemer, tx, purpose }) => {
  ...
  const ownInput = plet(
    pfind(PTxInInfo.type).$(( input: Term<typeof PTxInInfo> ) => {
        const convIn = plet( punsafeConvertType( input, PTxInInfo.type ) )
  ...
  if (!convRedeemer.r_product) {
    ...
    return passert.$(
      signedByBuyer.and(must_match_nonce).and(must_match_total).and(seller_must_be_compensated).and(buyer_must_get_deposit)
    );
  } else {
    ...
    if (pBs(keccak256(convRedeemer.r_product.toString()).toString('hex')).eq(datum.productHash)) {
      ...
      return passert.$(
      signedBySeller.and(signedByBuyer).and(must_match_nonce).and(must_match_total).and(seller_must_get_deposit).and(buyer_must_get_nothing)
      );
    } else {
      ...
      return passert.$(
      signedBySeller.and(signedByBuyer).and(must_match_nonce).and(must_match_total).and(seller_must_get_nothing).and(buyer_must_get_deposit)
      );
    }
  }
});
